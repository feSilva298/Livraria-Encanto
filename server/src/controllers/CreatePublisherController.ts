...
class CreatePublisherController {
  async handle(req: FastifyRequest, res: FastifyReply){
    ...
    const countCnpj = USE_DATABASE
      ? ((await databaseConnect.query(`SELECT COUNT(*) AS count FROM ${process.env.TABLE5} WHERE CNPJ = ?`, [CNPJ]))[0] as any)[0].count
      : await mockDb.editoras.countByCnpj(CNPJ);
    
    if (countCnpj > 0) {
      return res.status(401).send({ error: 'CNPJ já está cadastradoizado!' });;
    }

    const countEmail = USE_DATABASE
      ? ((await databaseConnect.query(`SELECT COUNT(*) AS count FROM ${process.env.TABLE5} WHERE email = ?`, [email]))[0] as any)[0].count
      : await mockDb.editoras.countByEmail(email);
