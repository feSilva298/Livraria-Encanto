...
class UploadBookImageController {
  async handle(req: FastifyRequest, res: FastifyReply) {
    ...
    try {
      const book = USE_DATABASE
        ? ((await databaseConnect.execute(`SELECT * FROM ${process.env.TABLE1} WHERE id = ?`, [id_livro]))[0] as any)[0]
        : await mockDb.livros.findById(Number(id_livro));
      ...
      if (USE_DATABASE) {
        ...
        await mockDb.livros.setImagem(Number(id_livro), fileName);
      }
